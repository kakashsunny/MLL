...
- Interactive walkthrough of a Jupyter challenge or code debugging!

Feel free to ask any question or paste a code snippet!`;
  }

  return `CONCEPTUAL DECONSTRUCTION: ${topic.toUpperCase()}

1. PHYSICAL INTUITION
Think of machine learning not as magic, but as fitting an elastic sheet over scattered data points in multi-dimensional space. The loss function measures how stretched or deformed the sheet is, and optimization algorithms gently pull on the corners until the sheet rests at minimum tension.

2. FIRST-PRINCIPLES BREAKDOWN (${mode.toUpperCase()} PERSPECTIVE)
Every supervised machine learning workflow fundamentally reduces to three components:
1. The Hypothesis: What mathematical family of functions f(x; theta) are we testing?
